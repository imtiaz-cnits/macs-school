...
<head>
    ...
    <title>Transfer Certificate - {{ $student->student_name }}</title>
    <style>
        @page {
            size: a4 landscape;
            margin: 0;
        ...
        * {
        ...
        /* Main Certificate Content Container (Tailored for TC) */
        .cert-container {
            position: relative;
            z-index: 5;
            padding: 17mm 26mm 14mm 26mm;
            text-align: center;
        }
...
<body>
    @php
        ...
        if (!isset($certNo) || !$certNo) {
            $certNo = 'MACS/TC/' . ($student->sessionYear->session_name ?? date('Y')) . '/' . str_pad($student->id, 4, '0', STR_PAD_LEFT);
        }
    ...
    <div class="cert-container">
        ...
        <!-- Certificate Title -->
        <div class="title-box">
            <span class="title-text">TRANSFER CERTIFICATE</span>
        </div>
        ...
        <p class="body-paragraph">
            Son/Daughter of <span class="field-highlight">{{ $student->father_name ?? '........................' }}</span> and <span class="field-highlight">{{ $student->mother_name ?? '........................' }}</span>, was a regular student of this prestigious institution in Class <span class="field-highlight">{{ $student->schoolClass->class_name ?? 'N/A' }}</span> under the academic session <span class="field-highlight">{{ $student->sessionYear->session_name ?? date('Y') }}</span>.
        </p>
        ...
        <p class="body-paragraph">
            He/She has appeared / passed the examination of Class <span class="field-highlight">{{ $student->schoolClass->class_name ?? 'N/A' }}</span> and secured the result: <span class="field-highlight">{{ $last_exam_result }}</span>.
        </p>

        <p class="body-paragraph">
            Reason for leaving the institution is stated as: <span class="field-highlight">{{ $leaving_reason }}</span>.
        </p>

        <p class="body-paragraph" style="margin-bottom: 10px;">
            To the best of our knowledge, he/she bears a good moral character. All institutional dues have been cleared. I wish him/her every success and a bright future in life.
        </p>
        ...
        <table class="authority-table">
            <tr>
                ...
                <td style="width: 32%; text-align: left;">
                    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 9.5px; font-weight: 700; color: #00558F; margin-bottom: 24px;">
                ...
                <td style="width: 32%; text-align: right;">
                    <div style="height: 36px; margin-bottom: 2px; text-align: center; margin-left: auto; width: 185px;">
                        @if($signatureSrc)
                            <img src="{{ $signatureSrc }}" style="height: 36px; object-fit: contain;" alt="Principal Signature" />
                        @endif
                    </div>
