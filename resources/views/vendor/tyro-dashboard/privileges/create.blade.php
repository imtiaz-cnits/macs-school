...
<div class="w-full min-h-screen">
    ...
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        <div>
            ...
            <p class="text-sm font-medium text-gray-555 dark:text-gray-400 mt-1">Add a new privilege that can be assigned to roles</p>
        </div>
        
        <a href="{{ route('tyro-dashboard.privileges.index') }}" class="h-11 px-6 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-750 text-gray-655 dark:text-gray-300 text-xs font-black rounded-xl uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-sm">
    ...
    <div class="bg-white dark:bg-themeNavy border border-gray-100 dark:border-white/[0.06] rounded-3xl p-6 md:p-8 mb-8 relative shadow-sm hover:shadow-md transition-all duration-300">
        <form action="{{ route('tyro-dashboard.privileges.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    <label for="name" class="text-[10px] font-black tracking-widest text-gray-555 dark:text-gray-400 uppercase mb-2 block">Privilege Name <span class="text-red-500 ml-0.5">*</span></label>
                    <input type="text" id="name" name="name" class="w-full h-11 border-2 border-gray-100 dark:border-gray-800 rounded-xl !bg-white dark:!bg-themeDark focus:outline-none focus:ring-4 focus:ring-themeBlue/10 focus:border-themeBlue transition-all text-sm font-semibold text-gray-700 dark:text-gray-250 px-3 placeholder-gray-400 @error('name') border-red-500 @enderror" value="{{ old('name') }}" required placeholder="e.g., Create Posts">
                    @error('name')
            ...
            <!-- Description Field -->
            <div class="mb-6">
                <label for="description" class="text-[10px] font-black tracking-widest text-gray-555 dark:text-gray-400 uppercase mb-2 block">Description <span class="text-gray-400 font-medium normal-case ml-1">(optional)</span></label>
                <textarea id="description" name="description" class="w-full border-2 border-gray-100 dark:border-gray-800 rounded-xl !bg-white dark:!bg-themeDark focus:outline-none focus:ring-4 focus:ring-themeBlue/10 focus:border-themeBlue transition-all text-sm font-semibold text-gray-700 dark:text-gray-250 p-3 placeholder-gray-400 @error('description') border-red-500 @enderror" rows="3" placeholder="Describe what this privilege allows users to do...">{{ old('description') }}</textarea>
                @error('description')
                    <span class="text-xs text-red-500 font-semibold mt-1.5 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-8 border-t border-gray-100 dark:border-white/[0.05] pt-6">
                <label class="text-[10px] font-black tracking-widest text-gray-555 dark:text-gray-400 uppercase mb-4 block">Assign to Roles</label>
                ...
                <div class="p-5 rounded-2xl bg-indigo-50/50 dark:bg-themeBlue/5 border border-indigo-100/50 dark:border-white/[0.04] flex items-start gap-3">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-gray-900 dark:text-white">No Roles Available</h4>
                        <p class="text-xs font-medium text-gray-555 dark:text-gray-400 mt-1">Please <a href="{{ route('tyro-dashboard.roles.create') }}" class="text-themeBlue hover:underline font-black">create a role first</a> before assigning this privilege.</p>
                    </div>
                </div>
                @endif
            ...
            <div class="flex gap-3 border-t border-gray-100 dark:border-white/[0.05] pt-6 justify-end">
                <a href="{{ route('tyro-dashboard.privileges.index') }}" class="h-11 px-6 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-750 text-gray-655 dark:text-gray-300 text-xs font-black rounded-xl uppercase tracking-wider transition-all flex items-center justify-center">Cancel</a>
                <button type="submit" class="h-11 px-8 bg-gradient-to-r from-themeBlue to-themeGreen text-white font-black rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all text-xs uppercase tracking-widest flex items-center justify-center whitespace-nowrap active:scale-95">Create Privilege</button>
            </div>
        </form>
    </div>
